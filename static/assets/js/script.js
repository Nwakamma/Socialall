...
function showMessage(type, message) {
    ...
    // Set the icon and class based on the type of message
    if (type === 'success') {
      flashIcon.textContent = '✔'; // Replace with icon from library if desired
      flashMessage.className = 'success';
    } else if (type === 'error') {
      flashIcon.textContent = '✖'; // Replace with icon from library if desired
      flashMessage.className = 'error';
    }
  ...
  // Call this function when the page loads or when you want to activate the preview
  previewImage();
...
function hides() {
    var edit_hide = document.getElementById('edit_hide');
    if (edit_hide.style.display === 'none') {
        edit_hide.style.display = 'block';
    } else {
        edit_hide.style.display = 'none';
    }
}
